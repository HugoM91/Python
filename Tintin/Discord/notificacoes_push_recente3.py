...
def start_var(sym):
    ...
    candles2 = client.get_klines(symbol=sym, interval=Client.KLINE_INTERVAL_30MINUTE, limit = 1)
    candles3 = client.get_klines(symbol=sym, interval=Client.KLINE_INTERVAL_30MINUTE, limit = 1)
    ...
    for x in candles3:
        ...
        hammer = indicadores(op, cl, hi, lo)[0]
        star = indicadores(op, cl, hi, lo)[1]
        hammer = hammer.tolist()
        star = star.tolist()
           
        if hammer[-1] == 100:
            webhook = DiscordWebhook(url_teste , content= (':green_circle: ' + sym + '//' + 'Hammer / 1 Minute'))
            response = webhook.execute()      
        if star[-1] == 100:
            webhook = DiscordWebhook(url_teste , content= (':red_circle: ' + sym + '//' + 'Shooting Star / 1 Minute'))
            response = webhook.execute()      
    ...
    for x in candles2[-1]:
        ...
        if hammer[-1] == 100:
            webhook = DiscordWebhook(url_teste , content= (':green_circle: ' + sym + '//' + 'Hammer / 30 Minute'))
            response = webhook.execute()      
        if star[-1] == 100:
            webhook = DiscordWebhook(url_teste , content= (':red_circle: ' + sym + '//' + 'Shooting Star / 30 Minute'))
            response = webhook.execute()  
