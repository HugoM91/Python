...
def run(a):
    ...
    print(prever(datapandas(a))[0])
    print('lista' , s)
    
    if len(s) == 2:
        if s[0] == r[0] and s[-1] == r[-1]:
            #startorder_open(a)
            bot.send_message(-475899940, 'Posicao aberta -- Sell -- Teste')
            k.append(1)
        
        
    if len(k) == 2:
        #startorder_tp(a)
        bot.send_message(-475899940, 'Posicao aberta --- sell / STOP --- Teste')
        del k[1]
