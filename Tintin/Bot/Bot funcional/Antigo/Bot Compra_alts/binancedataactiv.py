...
def run(a):
    print(prever(datapandas(a))[0] , str(a))
    ...
    if k >=15:
        print('Lista S = ' , s[a][-2] , s[a][-1], '/',  str(a))
        if s[a][-2] == r[0] and s[a][-1] == r[1]:
            bot.send_message(-442907326, text = ('Posicao aberta ---Teste --- alts' + str(a)))
