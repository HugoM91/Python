...
#---------------------------
    ...
    if op[-1] < cl[-1] and op[-2] < cl[-2] and op[-3] < cl[-3] and op[-4] < cl[-4] and op[-5] < cl[-5] and op_dif > 0.35:
        webhook = DiscordWebhook(patterns_webhook , content= (':green_circle:' + ' Five White Soldiers - 1 minute // ' + sym))
        response = webhook.execute()
    if op[-1] > cl[-1] and op[-2] > cl[-2] and op[-3] > cl[-3] and op[-4] > cl[-4] and op[-5] > cl[-5] and op_dif > 0.35:
        webhook = DiscordWebhook(patterns_webhook , content= (':red_circle:' + ' Five Black Crows - 1 minute // ' + sym))
        response = webhook.execute()
